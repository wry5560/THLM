<template>
	<div style="margin-top: 12px;">
		<div>
			...
			<span>
				等级：
				<a-select v-model:value="selectedLevel" style="width:100px" size="small">
		...
		<a-flex gap="small" wrap="wrap">
					...
					<a-tooltip :title="monster.name_en">
						<img-card :src="base+monster.images" style="width:86px" noMeta :title="monster.name" />
					</a-tooltip>
			...
			<div style="width:400px">
				<div style="width:100%;border:1px solid lightgray;">
					...
					<a-descriptions bordered :column="1">
						<a-descriptions-item label="怪物名称">{{ selectedMonster.name }}</a-descriptions-item>
						<a-descriptions-item label="等级">{{ selectedMonster.level }}</a-descriptions-item>
						<a-descriptions-item label="类型">{{ selectedMonster.type }}</a-descriptions-item>
					</a-descriptions>
				</div>
			</div>
		</a-flex>
...
const monsterData = computed(() => {
	...
	if(selectedType.value !== "All") tmp=tmp.filter((monster) => monster.type === selectedType.value);
	if(selectedLevel.value !== "All") tmp=tmp.filter((monster) => monster.level == selectedLevel.value)
